...
#data_holder = np.zeros((len(time_values), 100))
...
data_holder2 = np.zeros((5, len(time_values), number_of_samples))
...
for z, i in enumerate(data_holder[0,:,0]):
    ...
    #print(z, len(test0), len(test1), len(test2), len(test3), len(test4))
...
dominant_species = np.zeros(shape=(5,number_of_samples))
...
dom_spec = ['MoO4', 'Moo3S', 'MoO2S2', 'MoOS3', 'MoS4']

sum_spec = np.zeros((5))

for i in range(5):
    sum_spec[i] = np.sum(dominant_species[i])

ax.bar(dom_spec,sum_spec)
...
for i in range(0,number_of_samples):
    ...
    ls2, = ax.plot(time_values[:], data_holder2[0,:,i], c='grey', zorder=5, alpha=0.3)
    ls3, = ax.plot(time_values[:], data_holder2[1,:,i], c='goldenrod', zorder=4, alpha=0.3)
    ls4, = ax.plot(time_values[:], data_holder2[2,:,i], c='chocolate', zorder=3, alpha=0.3)
    ls5, = ax.plot(time_values[:], data_holder2[3,:,i], c='royalblue', zorder=2, alpha=0.3)
    ls6, = ax.plot(time_values[:], data_holder2[4,:,i], c='dodgerblue', zorder=1, alpha=0.3)
...
plt.ylim(-5,5)
plt.savefig('image_delta3.png', bbox_inches='tight', pad_inches=0.075, dpi=200, alpha=0.004)
#plt.show()
plt.close()
...
plt.ylim(-5,5)
plt.savefig('image_delta_mean.png', bbox_inches='tight', pad_inches=0.075, dpi=200, alpha=0.004)
#plt.show()
plt.close()
...
profile_graph = input('calculate water column profile?  ')
pub_test = np.zeros((5))
test_ratio = np.zeros((5))

if profile_graph == "yes":
    ...
    if test_ratio[4] - Mo_removal > 0:
        ...
        print(pub_test[1], pub_test[1] + iso_offset + abs(pub_test[0]))
    ...
    if test_ratio[4] - Mo_removal < 0:
        if test_ratio[3] + (test_ratio[4] - Mo_removal) < 0:
            ...
            print('two', pub_test[3], pub_test[3] + iso_offset)
            ...
            print('three', pub_test[2], pub_test[2] + iso_offset)
    else:
        pass
